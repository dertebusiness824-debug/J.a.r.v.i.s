...
3. **Tokens del ejecutor.** Los eventos `on_chat_model_stream` del ejecutor se reexpiden tal cual, también cuando el nodo anidado se llama `research_agent` (o el especialista que sea) y no `executor`. El planificador, el Supervisor y `retrieve` se silencian: su JSON estructurado no se puede pronunciar. Requiere un modelo con `streaming=True` (así se construye el ejecutor); en modo offline simplemente no hay tokens. Un `TokenGate` retiene unos pocos caracteres y descarta el stream si pinta a datos (`{`, `[`, ```` ``` ````, `<`) en vez de prosa. El planificador y el Supervisor se construyen con `disable_streaming=True`: dentro de `astream_events` LangChain pasa a stream cualquier `invoke`, y pedir `Plan`/`RouteDecision` por trozos es lo que hacía saltar el aviso `PydanticSerializationUnexpectedValue … field_name='parsed'` de `langchain-openai` y dejaba el JSON a medias si el stream se cortaba. En una sola petición no hay aviso ni trozos, y no se pierde nada porque esos tokens nunca se pronunciaban.
...
Si el grafo se queda mudo más de `VAPI_IDLE_SPEECH_SECONDS` (1,5 s), sale una frase de espera («Sigo en ello, maestro…») en vez de un keep-alive silencioso: el TTS de Vapi se vacía si solo llegan comentarios SSE. Cuando el ejecutor ya está hablando, una pausa entre tokens (típica tras una herramienta) solo manda `: keep-alive`: no se cierra con `[DONE]` ni se mete un «Sigo en ello» a mitad de frase. `GeneratorExit` / `CancelledError` se anotan como `🔌 [VAPI DISCONNECT]` (cortó Vapi) y no mandan el delimitador de fin. El ejecutor pide `max_tokens=2500` para que una explicación larga no se corte a medias. El Custom LLM responde siempre con `StreamingResponse` (`text/event-stream`) y `astream_events` del grafo; los especialistas reenvían el `RunnableConfig` del padre para que los tokens de OpenRouter no se queden atrapados en un `invoke` opaco. Con la voz activa, el presupuesto del turno puede ser largo: `VAPI_RESPONSE_TIMEOUT_SECONDS` son 75 s, por debajo del `timeoutSeconds` (90) que Vapi aplica al Custom LLM, para que una investigación larga acabe en lugar de tirarse a la basura a los 30 s. Si lo agota, se contesta una frase de espera y el turno tardío se descarta.
...
`POST /webhooks/vapi-llm` **siempre** responde con `StreamingResponse` (`text/event-stream`) cuando hay un turno del usuario. Los eventos de control que Vapi a veces manda al Custom LLM (`speech-update`, `status-update`, `transcript`, `metadata` y el resto de metadatos) se contestan con un JSON vacío `200 {}` y **no** invocan LangGraph ni escriben un WARNING: tratarlos como consulta vacía devolvía «Sistema listo.» en JSON síncrono y rompía la llamada. Si algo falla dentro del grafo, el endpoint sigue devolviendo 200 con deltas válidos y Jarvis dice «Mis sistemas han fallado, maestro» en voz alta (la traza completa queda en el log del servidor), en vez de cortar el stream y dejar la llamada muda.
